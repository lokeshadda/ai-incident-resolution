...
def _resolve_db_path() -> Path:
  """Project-local DB — writable when Streamlit runs inside Cursor/sandbox."""
  project_db = _ROOT / ".ai_incident_resolution" / "history.db"
  legacy_db = Path.home() / ".ai_incident_resolution" / "history.db"
  project_db.parent.mkdir(parents=True, exist_ok=True)
  if not project_db.exists() and legacy_db.is_file():
    try:
      import shutil

      shutil.copy2(legacy_db, project_db)
    except OSError:
      pass
  return project_db
